"""Expression baking for Kdenlive/MLT keyframe strings.

Uses the shared Expression class to evaluate a formula at every frame and
encode the results as a dense MLT keyframe string (same format as Shotcut
since both use the MLT backend).

Example usage::

    from cli_anything.kdenlive.core.expressions_mlt import bake_expression

    # Sinusoidal opacity
    kf_str = bake_expression("0.5 + 0.5 * sin(time * 6.28)", fps=25, duration=2.0)
    # → "00:00:00.000=0.500000;00:00:00.040=0.603553;..."
"""
...
from ..utils.mlt_xml import seconds_to_timecode  # noqa: E402
...
def bake_expression(
    ...
    Returns:
        ...
        ``"00:00:00.000=0.500000;00:00:00.040=0.591179;..."``
...
def bake_expression_frames(
    ...
    for frame in range(total_frames + 1):
        ...
        result.append({
            "frame": frame,
            "time": time,
            "timecode": seconds_to_timecode(time),
            "value": value,
        })
