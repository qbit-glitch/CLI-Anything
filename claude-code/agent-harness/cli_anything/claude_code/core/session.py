"""Claude Code CLI — Session management with undo/redo."""
...
class Session:
    """Manages conversation state with undo/redo history."""

    MAX_UNDO = 50

    def __init__(self):
        self.project: Optional[Dict[str, Any]] = create_conversation()
    ...
    def get_project(self) -> Dict[str, Any]:
        if self.project is None:
            raise RuntimeError("No conversation loaded.")
        return self.project
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "session_id": self.project.get("session_id") if self.project else None,
            "model": self.project.get("model") if self.project else None,
            "message_count": len(self.project.get("messages", [])) if self.project else 0,
        }
